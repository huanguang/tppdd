...
function setAllUAndGIdCache(){
    $user_ids   = UserModel::where('is_expired',0)->where('status','1')->column('id');
    $goods_ids  = GoodsModel::where('is_upper',1)->where('status','1')->column('id');
    $list = [];
    if($user_ids && $goods_ids)
    {
    ...
    Cache::set('user_goods_ids_cache',$list);
    return $list;
}

function getAllUAndGIdCache(){
    $list =  Cache::get('user_goods_ids_cache');
    if(!$list){
        $list = setAllUAndGIdCache();
    }
    return $list;
}
