...
class Pinduoduo extends Admin
{
        ...
        if ($this->request->isAjax()) {
            ...
            foreach ($list as $k=>$v) {
                ...
                $statusArr=[
                    ...
                    1=>'<span style="color:green">待发货</span>',
                    2=>'<span style="color:green">待收货</span>', 
                    3=>'<span style="color:green">待评价</span>',
                    4=>'<span style="color:red">交易已取消</span>',
                    ];
                $list[$k]['status'] = $statusArr[$list[$k]['status']];

                $list[$k]['is_notify'] = $v['is_notify'] == 1 ? '<span style="color:green">已通知</span>' : '否';
    ...
    private function send($order) {
        ...
        switch ($order['pay_type']) {
            case OrdersModel::WECHAT :
                $type = OrdersModel::$payments[OrdersModel::WECHAT];
                break;
            case OrdersModel::ALIPAY :
                $type = OrdersModel::$payments[OrdersModel::ALIPAY];
                break;
        }
