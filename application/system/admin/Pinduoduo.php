...
class Pinduoduo extends Admin
{
    ...
    public function orders()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            $notify_status = input('get.notify_status/s', '');
            ...
            if ($date) {
              ...
              if ($client) {
                $from_platform['c_id'] = $client;
                if ($client == -1) {
                    $from_platform['c_id'] = 1;
                }
                $query->where($from_platform);
            }
            ...
            if($notify_status){
                $query->where('is_pay','gt', 0 );
                $query->where('notify_status','<>', 1 );
            }
            $count = $query->count('id');
            ...
            foreach ($list as $k=>$v) {
                ...
                if($v['status'] > OrdersModel::UNPAY && $v['notify_status'] <> 1 ){
                    $list[$k]['notice'] = '<a href="'.url("Pinduoduo/sendNotify", ['id'=>$list[$k]['id']]).'" class="layui-btn layui-btn-warm layui-btn-xs notify">补发通知</a>';
                }
            }
    ...
    public function get_goods_info()
    {
        if ($this->request->isAjax()) {
            ...
            $goods_urls = $data['goods_url'];
            $goods_urls = array_unique(array_filter(explode("\n",$goods_urls)));
            if(count($goods_urls) > 20){
                return $this->ajax_error('最多添加20条链接!');
            }
            $rows = [];
	    unset($userModel);
            ...
            foreach ($goods_urls as $goods_url){
                $result = Tools::getGoods2($user,$goods_url, ADMIN_C_ID);
            ...
            $goodsModel->allowField(true)->insertAll($rows);
          	//var_dump($rows);die;
            //$goodsModel = new GoodsModel();
            //$goodsModel->isUpdate(false)->save($rows);
    ...
    public function goods_delall()
    {
        if ($this->request->isAjax()) {
            ...
            if ($result) {
                ...
                return $this->ajax_success('删除成功');
            } else {
                return $this->ajax_error('删除失败');
            }
    ...
    public function stores()
    {
        if ($this->request->isAjax()) {
            ...
            $list = StoresModel::with(['admin'=>function($query){
                $query->field(['id','nick']);
            }])->where($where)->order('status', 'desc')->page($this->page)->limit($this->limit)->select();
    ...
    public function stores_del()
    {
        ...
        $result = StoresModel::where(['id' => $id] + $where)->delete();
        if ($result) {
          	GoodsModel::where(['stores_id' => $id] + $where)->delete();
            return $this->success('删除成功');
        } else {
            return $this->error('删除失败');
        }
    ...
    public function stores_status()
    {
        if ($this->request->isAjax()) {
            ...
            if ($result) {
                 GoodsModel::where(['stores_id'=>$id])->setField('status',$status);
    ...
    public function users()
    {
        if ($this->request->isAjax()) {
			...
			$agent = input('get.agent/d', 0);
			$expired_limit_noaddr =  input('get.expired_limit_noaddr/d', 0);
			if($expired_limit_noaddr == 1){
				$where['expired_limit_noaddr'] = '<span style="color:green">正常</span>';
			}elseif($expired_limit_noaddr == 2){
				$where['expired_limit_noaddr'] = '<span style="color:red">超时</span>';
			}else{
				//$where['expired_limit_noaddr'] = false;
			}
			if ($agent) {
                $where['admin_uid'] = $agent;
            }
   ...
   /**
	...
	public function users_delall(){
		   if ($this->request->isAjax()) {
            ...
            $result = UserModel::where(['id' => $id] + $where)->delete();
  ...
  /**
    ...
    public function sendNotify(){
        $id = input('id/d', 0);
        $order = OrdersModel::where(['id'=>$id])->where('status','gt',OrdersModel::UNPAY)->where('notify_status','<>','1')->find();
        if($order && $order->c_id > 0) {
            $notify = new Notify('Notify');
            $notify->sendOrder($order);
            return $this->ajax_success('通知已发送');
        } else {
            return $this->ajax_error('订单不存在或已通知成功');
        }
    }
    ...
    public function onekey()
    {
        if ($this->request->isPost()) {
			...
			if(count($list) > 1000){
            	return $this->ajax_error('z最大导入量1000条数据');
            }
            foreach ($list as $v) {
