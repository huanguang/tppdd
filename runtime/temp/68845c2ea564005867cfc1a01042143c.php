...
<div class="layui-form">
    <div class="layui-form-item">
        ...
        <div class="layui-input-inline">
            <input type="text" class="layui-input layui-bg-gray" name="client_id" value="<?php echo htmlentities($user->client->client_id); ?>" readonly/>
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label">当前账户收入：</label>
        <div class="layui-input-inline">
            <input type="text" class="layui-input layui-bg-gray" value="<?php echo htmlentities($total); ?>" readonly/>
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label">提现金额：</label>
        <div class="layui-input-inline">
            <input type="number" class="layui-input" name="total" value="<?php echo htmlentities($cash_value); ?>"/><br>
            最大提现金额:<span style="color:red"><?php echo htmlentities($cash_value); ?></span>
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label">手续费：</label>
        <div class="layui-input-inline">
            <input type="text" class="layui-input layui-bg-gray" name="cash_fee" value="<?php echo htmlentities($cash_fee); ?>" readonly/>
        </div>
    </div>
    <div class="layui-form-item">
        <label class="layui-form-label">下发方式：</label>
        <div class="layui-input-inline">
            <?php if($user->banks->isEmpty()): ?>
            <blockquote class="layui-elem-quote layui-quote-nm layui-quote-diy">
                还没有代收银行信息，现在 <a href="<?php echo url('bank_add'); ?>" class="layui-btn layui-btn-danger">添加</a>
            </blockquote>
            <?php else: ?>
            <div class="layui-card">
                <div class="layui-card-header layui-bg-gray">选择代收银行</div>
                <div class="layui-card-body">
                    <?php if(is_array($user->banks) || $user->banks instanceof \think\Collection || $user->banks instanceof \think\Paginator): $k = 0; $__LIST__ = $user->banks;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?>
                    <div class="layui-form-item">
                        <div class="layui-input-inline">
                            <input type="radio" name="bank_id" value="<?php echo htmlentities($v->id); ?>" <?php echo $k==1 ? 'checked' : ''; ?> title="<?php echo htmlentities($v->title); ?>/<?php echo htmlentities($v->name); ?>">
                        </div>
                    </div>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                    <div class="layui-form-item">
                        <div class="layui-input-inline">
                            <a href="<?php echo url('bank_add'); ?>" class="layui-btn layui-btn-primary">+添加新的代收银行</a>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    layui.use(['jquery'], function() {
        ...
        $("#submitForm").on('click',function () {
            ...
            $.post('<?php echo url('pinduoduo/cash_add'); ?>',{
                total:total,
                bank_id:bank_id,
                ...
                if (res.status){
                    ...
                    self.location='<?php echo url('pinduoduo/my_settlement'); ?>';
                }else{
                    layer.msg(res.msg, {
                        time: 2000,
                        btn: ['知道了'],
        ...
        $('input[name=total]').on('blur', function(){
            ...
            $.get('<?php echo url('pinduoduo/cash_add'); ?>', {
                total: this.value
            }, function(res){
