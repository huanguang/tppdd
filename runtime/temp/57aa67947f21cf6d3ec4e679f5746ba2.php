...
<script type="text/html" title="状态模板" id="statusTpl">
    <input type="checkbox" name="status" value="{{ d.status }}" lay-skin="switch" lay-filter="switchStatus" lay-text="已支付|未支付" {{ d.status == 1 ? 'checked' : '' }} {{ d.status == 1 || d.status == 2 ? 'disabled' : '' }} data-href="<?php echo url('settlement_status'); ?>?id={{ d.id }}">
    <input type="checkbox" name="status" value="{{ d.status }}" lay-skin="switch" lay-filter="switchStatus" lay-text="已取消|未取消" {{ d.status == 2 ? 'checked' : '' }} {{ d.status == 1 || d.status == 2 ? 'disabled' : '' }} data-href="<?php echo url('settlement_cancel'); ?>?id={{ d.id }}">
</script>
...
<script type="text/javascript">
    layui.use(['table', 'jquery', 'layer'], function() {
        ...
        table.render({
            elem: '#dataTable'
            ,url: '<?php echo url('settlement'); ?>' //数据接口
            ,page: true //开启分页
            ,limit: 10
            ,skin: 'row'
            ,even: true
