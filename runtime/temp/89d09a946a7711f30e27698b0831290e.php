...
<script type="text/html" id="toolbar">
    <div class="layui-btn-group fl">
        <a data-href="<?php echo url('stores_status?val=1'); ?>" class="layui-btn layui-btn-primary layui-btn-sm hisi-page-btns layui-icon layui-icon-play" data-table="dataTable">&nbsp;启用</a>
        <a data-href="<?php echo url('stores_status?val=0'); ?>" class="layui-btn layui-btn-primary layui-btn-sm hisi-page-btns layui-icon layui-icon-pause" data-table="dataTable">&nbsp;禁用</a>
        <a data-href="<?php echo url('stroes_delall'); ?>" class="layui-btn layui-btn-primary layui-btn-sm hisi-page-btns layui-icon layui-icon-delete" data-table="dataTable">&nbsp;删除</a>
    </div>
</script>
...
<script>
    layui.use(['form','jquery','table'], function(){
        ...
        table.render({
            elem: '#dataTable'
            ,url:'<?php echo url('pinduoduo/stores'); ?>'
            ,page: true
            ,cellMinWidth: 80
            ,toolbar: '#toolbar'
            ,cols: [[
        ...
        table.on('edit(dataTable)', function(obj){
            ...
            if (field=='name'){
                ...
                if (flag) {
                    $.ajax({
                        type:"POST",
                        url: "<?php echo url('pinduoduo/stores_rename'); ?>",
                        data:{
                            id:id
                            ,name:name
                        },
            ...
            if (field=='store_remain_total') {
                ...
                let flag = true;
                if (flag) {
                    $.post('<?php echo url('pinduoduo/stores_remain_total'); ?>', {
                        id: id,
        ...
        form.on('switch(StatusSwitch)', function(data){
            ...
            $.post('<?php echo url('pinduoduo/stores_status'); ?>',{
                id:id,
                val:status
            },function (res) {
                console.log(res);
  	...
  	function stores_del(id) {
        if (confirm('确认是否删除？')==true){
            self.location='<?php echo url('pinduoduo/stores_del'); ?>?id='+id;
        }
    }
</script>
                    </div>
