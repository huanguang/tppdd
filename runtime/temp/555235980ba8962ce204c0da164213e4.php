...
<script type="text/html" title="状态模板" id="statusTpl">
    {{ d.status == 1 ? '<span style="color:green;">已付款</span>' : '<span>未付款</span>' }}
    &nbsp;&nbsp;
    <?php if(false): ?><input type="checkbox" name="status" value="{{ d.status }}" lay-skin="switch" lay-filter="switchStatus" lay-text="已取消|未取消" {{ d.status == 2 ? 'checked' : '' }} {{ d.status == 1 || d.status == 2 ? 'disabled' : '' }} data-href="<?php echo url('my_settlement_cancel'); ?>?id={{ d.id }}"><?php endif; ?>
    {{ d.status == 2 ? '<span style="color:red;">已取消</span>' : '<span>未取消</span>' }}
</script>
...
<script type="text/javascript">
    layui.use(['table', 'jquery', 'layer'], function() {
        ...
        table.render({
            elem: '#dataTable'
            ,url: '<?php echo url('my_settlement'); ?>' //数据接口
            ,page: true //开启分页
            ,limit: 10
            ,skin: 'row'
            ,even: true
